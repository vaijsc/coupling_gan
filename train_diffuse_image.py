...
def train(rank, gpu, args):
    ...
    parent_dir += f'/diffuseimage/{args.phi1}_{args.phi2}/{args.image_distance}/tau_{args.tau}/std_{args.diffuse_std}'
    ...
    ### Warning, might be wrong codes due to random_sampler, it makes duplicates 
    for batch in inject_dataloader:
        all_origin_images, _ = batch     
        all_origin_images = all_origin_images.to(device, non_blocking=True)
        break

    normal_distribution = torch.distributions.Normal(loc=all_origin_images, scale= args.diffuse_std * torch.ones_like(all_origin_images))
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for _, x in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            sampled_indices = torch.randperm(len_inject_dataset)[:batch_size]
            sampled_values = normal_distribution.sample().detach()

            # sampled_values = normal_distribution.sample().detach()
            # indices_with_duplicates = torch.randint(0, len_inject_dataset, size=(batch_size,))
            noise = sampled_values[sampled_indices]   
