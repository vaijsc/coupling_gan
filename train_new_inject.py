...
def train(rank, gpu, args):
    ...
    if args.inject_model:
        parent_dir += f'/{args.inject_type}_{args.inject_model}'
    ...
    if args.inject_model:
        inject_model_path = f"./train_logs/{args.inject_type}/{args.noise_type}/linear_linear/l2/tau_0.001/std_{args.diffuse_std}/nn/netG/netG_{args.inject_model}.pth"
    ...
    #     ckpt = torch.load(inject_model_path, map_location=device)
    ...
    if args.resume:
        ...
        netG.load_state_dict(torch.load(inject_model_path))
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for _, x in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            real_data = x.float().to(device, non_blocking=True) - batch_mean_images
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inject_model', type=str, default=None, help='type of initial noise to generate')
    parser.add_argument('--inject_type', type=str, default=None, help='type of initial noise to generate')
