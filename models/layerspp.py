...
class AdaptiveGroupNormNoLatent(nn.Module):
...
class ResnetBlockBigGANpp_Adagn_No_Latent(nn.Module):
  def __init__(self, act, in_ch, out_ch=None, up=False, down=False,
    ...
    out_ch = out_ch if out_ch else in_ch
    # self.GroupNorm_0 = AdaptiveGroupNormNoLatent(min(in_ch // 4, 32), in_ch)
    ...
    # self.GroupNorm_1 = AdaptiveGroupNormNoLatent(min(out_ch // 4, 32), out_ch)
