...
from utils import *
from dataset import get_dataset, get_dataset_pair
...
def train(rank, gpu, args):
    ...
    dataset_pair = get_dataset_pair(args)
    train_sampler_pair = torch.utils.data.distributed.DistributedSampler(dataset_pair,
                                                                    num_replicas=args.world_size,
                                                                    rank=rank)
    data_loader_pair = torch.utils.data.DataLoader(dataset_pair,
                                               batch_size=batch_size,
                                               shuffle=False,
                                               num_workers=4,
                                               pin_memory=True,
    ...
    global get_image_loss
    get_image_loss = get_image_loss(args.image_loss_type)

    if args.stop_image_loss_from < 0:
        epoch_stop_image_loss = args.num_epoch + 5
    else:
        epoch_stop_image_loss = args.stop_image_loss_from
    if args.inverse_alpha_from < 0:
        epoch_inverse_alpha = args.num_epoch + 5
    else:
        epoch_inverse_alpha = args.inverse_alpha_from
    ...
    max_inv_alpha = args.max_inv_alpha
    if init_epoch <= epoch_inverse_alpha:
        for i in range(init_epoch):
            alpha *= alpha_decay
    else:
        for i in range(epoch_inverse_alpha):
            alpha = alpha * alpha_decay
        for i in range(epoch_inverse_alpha, init_epoch):
            alpha /= alpha_decay
            alpha = min(max_inv_alpha, alpha)

    for epoch in range(init_epoch, args.num_epoch+1):
        # print(f'Epoch {epoch}')
        train_sampler.set_epoch(epoch)
        
        for x, x_pair in tqdm(zip(data_loader, data_loader_pair), total=len(data_loader)):
            # try: x,_ = x
            # except: pass
            x,_ = x
            x_pair, noise_pair, label_pair = x_pair
            ...
            errG = (args.tau * torch.sum(((x_0_predict-noise).view(noise.size(0), -1))**2, dim=1) - D_fake).mean()
            # errG = errG.mean()

            if epoch < epoch_stop_image_loss:
                # Coupling data
                real_data_pair = x_pair.float().to(device, non_blocking=True)
                # real_data_pair.requires_grad = True
                noise_pair = noise_pair.float().to(device, non_blocking=True)

                x_0_predict_pair = netG(noise_pair)
                # image_loss(x_0_predict_pair, real_data_pair, reduction='none')
                errG_pair = get_image_loss(x_0_predict_pair, real_data_pair).mean()

                if discriminator_loss_weight > 0:
                    predict_pair_features = netD.module.forward_features(x_0_predict_pair)
                    real_pair_features = netD.module.forward_features(real_data_pair)
                    errG_pair_features = torch.mean(torch.abs(predict_pair_features - real_pair_features))
                    errG_pair = discriminator_loss_weight * errG_pair_features + (1 - discriminator_loss_weight) * errG_pair
                
                err = alpha * errG + (1 - alpha) * errG_pair * args.image_loss_weight
            else:
                errG_pair = torch.zeros(1)
                err = errG
            ...
            if global_step % args.print_every == 0:
                log_content = f'Epoch {epoch:04d} : G Loss {errG.item():.4f}, Image Loss {errG_pair.item():.4f}, D Loss {errD.item():.4f}, Elapsed {datetime.now() - start}'
                print(log_content)
        ...
        if rank == 0:
            ...
            if epoch % args.save_content_every == 0:
                ...
                if epoch % args.save_content_epoch_every == 0 and epoch >= args.save_content_from_epoch:
                    torch.save(content, os.path.join(content_path, f'content_{epoch}.pth'))
            
            # save checkpoint
            if epoch % args.save_ckpt_every == 0 and epoch >= args.save_ckpt_from_epoch:
                if args.use_ema:
            ...
            if epoch % args.save_image_every == 0:
                data = np.load(f'data/{args.datasetpair}_64/pairs/pair.npz')
                noise = torch.from_numpy(data['latent']).float().to(device, non_blocking=True)
                label = torch.from_numpy(data['label']).float().to(device, non_blocking=True)
                # noise = torch.from_numpy(np.load(f'data/{args.datasetpair}_64/pairs/pair.npy'))[1, :, :, :, :].float().to(device, non_blocking=True)
                images = netG(noise)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_loss_weight', type=float, default=6, help='weight of the image loss')
    parser.add_argument('--stop_image_loss_from', type=int, default=-1, help='stop image loss from an epoch')
    parser.add_argument('--inverse_alpha_from', type=int, default=-1, help='inverse alpha from an epoch')
    parser.add_argument('--max_inv_alpha', type=float, default=1, help='maximum of inverse alpha')
    parser.add_argument('--image_loss_type', type=str, default='l1', help='type of loss for image')
    parser.add_argument('--discriminator_loss_weight', type=float, default=0, help='weight of discriminator loss')
    # parser.add_argument('--l1_amplification', type=float, default=1.001, help='amplification of l1 weight')
    # parser.add_argument('--stop_l1', type=float, default=300, help='Epoch id when models deactivate l1 loss')
    ...
    parser.add_argument('--save_content_from_epoch', type=int, default=50, help='save content from an epoch')
    parser.add_argument('--save_ckpt_from_epoch', type=int, default=-1, help='save ckpt from an epoch')
