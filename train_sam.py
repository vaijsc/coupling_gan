...
def train(rank, gpu, args):
    ...
    optimizerG = SAM(netG.parameters(), optim.Adam, lr=args.lr_g, betas = (args.beta1, args.beta2))
    ...
    start = datetime.now()
    alpha = args.alpha
    dfeature_weight = args.dfeature_weight
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for x, x_pair in tqdm(zip(data_loader, data_loader_pair), total=len(data_loader)):
            ...
            # l1_loss(x_0_predict_pair, real_data_pair, reduction='none')
            errG_pair = torch.mean(torch.abs(x_0_predict_pair - real_data_pair))

            err = alpha * errG + (1 - alpha) * errG_pair * args.l1_weight

            # first forward-backward pass
            err.backward()
            optimizerG.first_step(zero_grad=True)
            ...
            err = alpha * errG + (1 - alpha) * errG_pair * args.l1_weight
            err.backward()  # make sure to do a full forward pass
            optimizerG.second_step(zero_grad=True)
            ...
            if global_step % args.print_every == 0:
                log_content = f'Epoch {epoch:04d} : G Loss {errG.item():.4f}, L1 Loss {errG_pair.item():.4f}, D Loss {errD.item():.4f}, Elapsed {datetime.now() - start}'
                print(log_content)
        ...
        if rank == 0:
            ...
            if epoch % args.save_image_every == 0:
                ...
                torchvision.utils.save_image(images, os.path.join(exp_path, 'epoch_{}.jpg'.format(epoch)))
