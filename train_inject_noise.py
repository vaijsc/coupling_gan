...
def train(rank, gpu, args):
    ...
    parent_dir += f'/injectnoise/{args.phi1}_{args.phi2}/{args.image_distance}/tau_{args.tau}/inject_{args.inject_noise_ratio}'
    ...
    # inject_dataloader = torch.utils.data.DataLoader(inject_dataset, batch_size=batch_size, num_workers=4, pin_memory=True, sampler=random_sampler, drop_last = True)
    len_inject_dataset = len(inject_dataset)
    inject_dataloader = torch.utils.data.DataLoader(inject_dataset, batch_size=len_inject_dataset, num_workers=4, pin_memory=True, sampler=random_sampler, drop_last = True)
    ...
    inject_noise_ratio = args.inject_noise_ratio
    origin_image_ratio = 1 - inject_noise_ratio
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for _, x in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # for batch in inject_dataloader:
            #     origin_images, _ = batch     
            #     origin_images = origin_images.to(device, non_blocking=True)
            #     break

            indices_with_duplicates = torch.randint(0, len_inject_dataset, size=(batch_size,))
            origin_images = all_origin_images[indices_with_duplicates]

            noise = origin_image_ratio * origin_images + inject_noise_ratio * torch.randn_like(real_data)    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inject_noise_ratio', type=float, default=1, help='ratio of noise to each image')
