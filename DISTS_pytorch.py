...
class DISTS(torch.nn.Module):
    def __init__(self, load_weights=True):
        ...
        vgg_pretrained_features = models.vgg16(pretrained=True).features.cuda()
        ...
        self.register_buffer("mean", torch.tensor([0.485, 0.456, 0.406]).view(1,-1,1,1).cuda())
        self.register_buffer("std", torch.tensor([0.229, 0.224, 0.225]).view(1,-1,1,1).cuda())

        self.chns = [3,64,128,256,512,512]
        self.register_parameter("alpha", nn.Parameter(torch.randn(1, sum(self.chns),1,1).cuda()))
        self.register_parameter("beta", nn.Parameter(torch.randn(1, sum(self.chns),1,1).cuda()))
